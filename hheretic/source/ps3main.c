/* hheretic for PS3- ported by Hermes */
...
void My_Quit(void) {
    ...
    ioPadSetActDirect(last_rumble, &actparam);
   ...
   // VIRTUAL_HEIGHT = 512;
        ...
        if(argv && !strncmp((char *) argv[0], "/dev_" , 5)) {
            ...
            if(*p == '/') {
                ...
                if(fp) {
                    ...
                    basePath = "/dev_usb/heretic/"; // by default use this folder
    ...
    for(n=0;n<150;n++) {
        ...
        if((new_pad & BUTTON_CIRCLE) && strncmp(my_wad, "/dev_usb", 8)) {
            copyfiles("/dev_usb/heretic/", my_wad);
            basePath = my_wad; 
            break;
        }
        usleep(20000);
...
void S_StartSong(int song, BOOLEAN loop)
{
	...
	isExternalSong = 0;//I_RegisterExternalSong(S_music[song].name);
	if (isExternalSong)
	{
...
void ps3_scr_update()
{
    ...
    if(ps3_scr_info & 1)
        ...
        PY+=32;PX=8;
        s_printf("Button CROSS -> Change Weapon");
        ...
        PY+=32;PX=8;
        s_printf("LEFT/RIGHT -> ZOOM MAP");

        PY+=32;PX=8;
        s_printf("UP/DOWN -> for fly. With L2+CIRCLE fly Off");
...
#define MACRO_EVENT(x, y) \
    event.data1 = y; \
    if(x) { \
       if(last_event[nevent]!=1) {event.type = ev_keydown;D_PostEvent(&event); last_event[nevent]=1;} \
    } \
    else { \
       if(last_event[nevent]==1) {event.type = ev_keyup;D_PostEvent(&event); last_event[nevent]=-1;} \
    }\
    nevent++;
...
void I_StartFrame (void)
{
    ...
    int k_weapon=0,k_leftsel=0,k_rightsel=0,k_alt=0, k_space=0, k_plus=0, k_minus=0;
    //int k_1=0,k_2=0,k_3=0,k_4=0,k_5=0,k_6=0;
    int k_fly=0;
...
//    if(new_pad & BUTTON_START) k_enter = 1;
    ...
    if(!ps3_scr_info) {
        ...
        if(new_pad & BUTTON_CROSS) ev.data1|=k_weapon=1;  // jump
...
//        if(old_pad & BUTTON_L2)
    ...
    if((ps3_scr_info & 1) && (old_pad & BUTTON_L1) && use_cheat>4 && usergame)
        { 
        if(new_pad & BUTTON_UP) {new_pad &=~ BUTTON_UP; set_my_cheat(2);ps3_scr_info&=2;}
        if(new_pad & BUTTON_RIGHT) {new_pad &=~ BUTTON_RIGHT; set_my_cheat(0);ps3_scr_info&=2;}
        if(new_pad & BUTTON_DOWN) {new_pad &=~ BUTTON_DOWN; set_my_cheat(4);ps3_scr_info&=2;}
        if(new_pad & BUTTON_LEFT) {new_pad &=~ BUTTON_LEFT; set_my_cheat(5);ps3_scr_info&=2;}
        if(new_pad & BUTTON_TRIANGLE) {new_pad &=~ BUTTON_TRIANGLE; set_my_cheat(10);ps3_scr_info&=2;}
        }
    else
